...
ID3D12Resource* ParticleCommon::CreateBufferResource(size_t sizeInBytes)
{
	//頂点リソース用のヒープの設定
...
ParticleCommon::MaterialData ParticleCommon::LoadMaterialTemplateFile(const std::string& directoryPath, const std::string& fileName)
{
	// 1. 中で必要となる変数の宣言
...
ParticleCommon::MeshData ParticleCommon::LoadObjeFile(const std::string& directoryPath, const std::string& fileName)
{
	// 1. 中で必要となる変数の宣言
...
uint32_t ParticleCommon::LoadObj(const std::string& directoryPath, const std::string& fileName)
{

	for (uint32_t texNum = 0; texNum < static_cast<uint32_t>(meshs_.size()); texNum++) {

		if (meshs_[texNum].directoryPath == directoryPath && meshs_[texNum].fileName == fileName) {
			return texNum;
		}
	}

	meshs_.push_back(LoadObjeFile(directoryPath, fileName));

	meshs_.back().directoryPath = directoryPath;
...
void ParticleCommon::InitializeDXC()
{
	//DXCの初期化
...
void ParticleCommon::InitializePSO()
{
	...
	//Shaderをコンパイルする
	vertexShaderBlob_ = CompileShader(L"Resources/Shaders/Particle.VS.hlsl", L"vs_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
	assert(vertexShaderBlob_ != nullptr);

	pixelShaderBlob_ = CompileShader(L"Resources/Shaders/Particle.PS.hlsl", L"ps_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
...
IDxcBlob* ParticleCommon::CompileShader(const std::wstring& filePath, const wchar_t* profile, IDxcUtils* dxcUtils, IDxcCompiler3* dxcCompiler, IDxcIncludeHandler* includeHandler)
{
	// 1. hlslファイルを読む

#include "BlockGraphicsPipeline.h"
...
BlockGraphicsPipeline* BlockGraphicsPipeline::GetInstance()
{
	static BlockGraphicsPipeline instance;
	return &instance;
}

void BlockGraphicsPipeline::Initialize()
{
...
void BlockGraphicsPipeline::PreDraw()
{
...
void BlockGraphicsPipeline::SetBlendMode(uint32_t blendMode)
{
...
void BlockGraphicsPipeline::InitializeDXC()
{
	//DXCの初期化
...
void BlockGraphicsPipeline::InitializePSO()
{
	//DescriptorRange
	...
	//Shaderをコンパイルする
	vertexShaderBlob_ = CompileShader(L"Resources/Shaders/Blocks.VS.hlsl", L"vs_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
	assert(vertexShaderBlob_ != nullptr);

	pixelShaderBlob_ = CompileShader(L"Resources/Shaders/Blocks.PS.hlsl", L"ps_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
...
IDxcBlob* BlockGraphicsPipeline::CompileShader(const std::wstring& filePath, const wchar_t* profile, IDxcUtils* dxcUtils, IDxcCompiler3* dxcCompiler, IDxcIncludeHandler* includeHandler)
{
	// 1. hlslファイルを読む

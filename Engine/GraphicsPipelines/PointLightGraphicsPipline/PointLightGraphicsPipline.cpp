...
PointLightGraphicsPipline* PointLightGraphicsPipline::GetInstance()
{
	static PointLightGraphicsPipline instance;
	return &instance;
}
...
void PointLightGraphicsPipline::PreDraw()
{
...
void PointLightGraphicsPipline::SetBlendMode(uint32_t blendMode)
{
...
void PointLightGraphicsPipline::InitializeDXC()
{
	//DXCの初期化
...
void PointLightGraphicsPipline::InitializePSO()
{
	...
	//Shaderをコンパイルする
	vertexShaderBlob_ = CompileShader(L"Resources/Shaders/PointLight.VS.hlsl", L"vs_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
	assert(vertexShaderBlob_ != nullptr);

	pixelShaderBlob_ = CompileShader(L"Resources/Shaders/PointLight.PS.hlsl", L"ps_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
...
IDxcBlob* PointLightGraphicsPipline::CompileShader(const std::wstring& filePath, const wchar_t* profile, IDxcUtils* dxcUtils, IDxcCompiler3* dxcCompiler, IDxcIncludeHandler* includeHandler)
{
	// 1. hlslファイルを読む
